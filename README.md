...
## ✨개요
- **POOKY**는 Rasberry pi를 기반으로 6개의 모터 연결하여 제작한 6축 로봇팔입니다. flask를 사용해 웹페이지를 통해 모터를 제어하고 기구학을 통해 구한 X, Y, Z, Rx, Ry, Rz를 사용자가 웹에서 확인할 수 있습니다.
...
## 🎯목표
- **6축 로봇팔 설계 및 구현** : Inventor를 활용해 6축 로봇팔의 구조를 3D 모델링하고, 시뮬레이션을 통해 설계의 안정성과 작동 범위를 검증
- **정/역기구학 구현 및 추적** : Matlab을 사용해 정기구학 및 역기구학 알고리즘을 개발하고, Joint 값 또는 위치 좌표(X, Y, Z, Rx, Ry, Rz)를 정밀하게 계산 및 추적
- **웹 기반 제어 시스템 개발** : Raspberry Pi와 Flask를 이용해 로봇팔을 웹과 연동, 사용자가 웹 인터페이스를 통해 로봇팔을 원격으로 제어하고 실시간 상태를 모니터링할 수 있는 시스템 구축
...
### DH Parameter
모터의 회전축을 Z축으로 기준 삼아 Modified DH Parameter를 계산했습니다.
...
각 GPIO핀에 연결된 모터에 PWM신호를 보내 모터가 원하는 각도(-90 ~ 90)로 움직일 수 있도록 했습니다.
...
Python 기반의 웹 프레임워크 **Flask**를 사용하여 웹을 구현<br>
클라이언트(라즈베리파이)와 서버 간의 데이터(로봇의 Joint값, 위치/회전 데이터)을 주고 받았습니다.

## 6축로봇 제작

### 3D 프린터를 통해 제작

|1. Base & Base_cover|2. Base_shoulder|3. Arm1 & Arm_cover|
| --- | --- | --- |
|<img src="Base&Base_cover.jpeg" width="800"> | <img src="Base_shoulder.jpeg" width="800">|<img src="Arm1&Arm_cover.jpeg" width="800"> | 

|4. amr1_shoulder|5. Arm2 & Arm_cover|6. arm2_shoulder & End-effector|
| --- | --- | --- |
| <img src="amr1_shoulder.jpeg" width="800">|<img src="Arm2&Arm_cover.jpeg" width="800"> | <img src="arm2_shoulder&End-effector.jpeg" width="800">|

### 회로 연결
<div align="center">
    <img src="img_1.jpeg" width="1200">
</div>

### 시연

[Watch the video](https://github.com/Hwang-06/POOKY_PJT/blob/master/test_1.mov)

## Conculsion

6축로봇을 설계부터 제어 및 웹구현 까지 완성했다. 각 팀원들이 정해진 역할을 맡아 로봇 설계 및 제작, 로봇 제어 알고리즘 구현, 원격제어 및 웹페이지 구현을 수행했습니다.

라즈베리파이를 사용해 각각의 모터를 제어했으며 Joint값과 정/역기구학을 통해 구한 위치/회전 데이터를 Flask를 통해 동적으로 HTML과 주고 받았습니다.

MG996R모터의 회전 범위 -90° ~ 90°사이에서 원하는 각도를 입력했을 때 6축로봇이 움직이는 것을 확인할 수 있으며 HTML 웹페이지에서 계산한 위치/회전 데이터가 Matlab을 통해 계산한 위치/회전 데이터와 일치하는 것을 확인했습니다.

|1. Screen|2. Matlab|
| --- | --- |
|<img src="screen_1.gif" width="1000"> | <img src="MATLABR.gif" width="1000">| 

앞으로 프로젝트를 진행하며 보완해야 할 점이 있습니다.

- **MG996R 서보모터 교체**

현재 사용 중인 MG996R 서보모터는 -90° ~ 90°의 제한된 회전 범위를 제공하며, 낮은 회전 토크로 인해 무거운 부품을 지탱하지 못해 2번 Joint의 원활한 작동이 어려운 상황입니다. 이러한 제약을 해결하기 위해 -180° ~ 180°의 확장된 회전 범위와 높은 회전 토크를 제공하는 서보모터로의 교체가 필요합니다. 이를 통해 로봇의 동작 범위 제약을 최소화하고, 모든 Joint가 제약 없이 회전할 수 있는 환경을 구현할 수 있습니다.

- **기구학 알고리즘 개선**

HTML 환경에서 정/역기구학 알고리즘을 활용하여 Joint와 위치/회전 데이터 간의 변환을 시도하였으나, 역기구학 계산의 불안정성이 관찰되었습니다. 특히, 파이썬과 HTML 간 Math 라이브러리의 차이로 인해 Matlab에서의 계산값과 HTML에서의 계산값이 일치하지 않는 문제가 발생하였습니다. 이를 개선하기 위해 정교한 알고리즘 최적화와 데이터 일관성을 확보할 수 있는 새로운 수학적 접근 방식을 도입할 필요가 있습니다.

- **웹페이지 구성 및 알고리즘 최적화**

6축 로봇의 작업 환경은 잠재적인 위험 요소를 동반합니다. 현재 로봇의 움직임 속도를 제어할 수 없는 한계가 있으며, 속도 조절 기능을 추가하여 사용자 안전을 보장해야 합니다. 또한, 로봇 제어 시 관리자 외 사용을 제한하기 위해 웹 기반 제어 시스템에 적절한 인증 절차를 도입해야 합니다. 이를 통해 관리자만 로봇을 조작할 수 있도록 보안성을 강화하고, 안전하고 신뢰할 수 있는 로봇 제어 환경을 구축할 수 있습니다.
